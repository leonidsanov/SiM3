...
interval   = INTERVAL_D1 -- В данный момент интервал 1 день
------------------
-- Тело скрипта --
------------------
...
function main()
	while IsRun do
		if os.sysdate().sec == 30 then                      -- Каждые 15 секунд обновляет файл с данными, полученными с сервера (внимание!), поэтому если данные с сервера не пришли файл будет пустой даже если в терминале есть график
			CSV = io.open(getScriptPath() .. "/SiM3_Data_D1.csv", "w"); -- Создает, или открывает для чтения/добавления файл CSV в той же папке в режиме с возможностью записи, позволяющий изменять существующие файлы и создавать новые файлы (нечитаемые), где находится данный скрипт
