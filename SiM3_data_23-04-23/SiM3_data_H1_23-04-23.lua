...
SEC_CODE   = "SiM3";     -- Код бумаги
interval   = INTERVAL_H1 -- В данный момент интервал 60 минут
------------------
-- Тело скрипта --
------------------
...
function main()
	while IsRun do
		if os.sysdate().sec == 30 then                      -- Каждые 15 секунд обновляет файл с данными, полученными с сервера (внимание!), поэтому если данные с сервера не пришли файл будет пустой даже если в терминале есть график
			CSV = io.open(getScriptPath() .. "/SiM3_Data_H1.csv", "w"); -- Создает, или открывает для чтения/добавления файл CSV в той же папке в режиме с возможностью записи, позволяющий изменять существующие файлы и создавать новые файлы (нечитаемые), где находится данный скрипт
			...
			for i = Size, 1, -1 do
				CSV:write(
					tostring(ds:T(i).year) ..
					'-' ..
					tostring(ds:T(i).month) ..
					'-' ..
					tostring(ds:T(i).day) ..
					',' ..
					tostring(ds:T(i).hour) ..
					':' ..
					tostring(ds:T(i).min) ..
					',' ..
					tostring(ds:O(i)) ..
					',' .. tostring(ds:H(i)) .. ',' .. tostring(ds:L(i)) .. ',' .. tostring(ds:C(i)) ..
